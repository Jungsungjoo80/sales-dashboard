# -*- coding: utf-8 -*-
"""
LABONLAB 트래픽 데이터 분석 대시보드 v7

반영된 수정(요청 2개):
1) 주차별 통합 시각화가 표시 안 되는 문제 해결
   - 필터 결과/집계 결과가 비어있을 때 원인 메시지 표시
   - column-selector 미선택 시 기본 컬럼 자동 선택
2) 전체 라벨 표시 시 숫자가 안 보이는 문제 해결
   - label_mode='all'일 때 uniformtext 숨김 정책 해제
   - 공통 margin 확대 + cliponaxis=False로 잘림 방지

추가 기능(이전 요청 반영 유지):
- 상품 필터(멀티 선택) 추가: 테이블/통합시각화/개별차트 모두 연동
- 라벨 표시 방식: 상위5/전체/없음
"""
...
def apply_common_figure_style(fig, height=500, label_mode='top'):
    """
    - 상단 마진 확장(라벨 잘림 방지)
    - label_mode='all'일 때 uniformtext 숨김 정책 해제(라벨이 사라지는 문제 방지)
    """
    if label_mode == 'all':
        uniform_mode = False
        uniform_min = 8
    ...
    fig.update_layout(
        height=height,
        margin=dict(t=220, b=140, l=80, r=60),
        hovermode='x unified',
        uniformtext_minsize=uniform_min,
        uniformtext_mode=uniform_mode,
...
def get_label_text_and_position(values, col, label_mode, top_n=5):
    """
    label_mode:
      - 'all': 모든 값 text 표시
      - 'top': 상위 N개만 text 표시
    ...
    if col in ['매출', '이익', '트래픽비용', '순이익']:
        texts = [format_currency_short_man(v) for v in values]
    elif col in ['이익률', '이익률변동', 'ROAS']:
        texts = [format_percent(v) for v in values]
    else:
        texts = [f"{int(v):,}" if pd.notna(v) else "0" for v in values]

    if label_mode == 'none':
        return texts, 'none'

    if label_mode == 'all':
        return texts, 'outside'

    # top
    s = pd.Series(values).fillna(0)
    top_idx = set(s.nlargest(min(top_n, len(s))).index.tolist())
    masked = [texts[i] if i in top_idx else "" for i in range(len(texts))]
...
def upload_file(contents, filename):
    ...
    if error:
        return None, None, None, None, html.Div([html.Span(error, className="text-danger")])
...
def update_mall_filter(malls):
    ...
    opts = [{'label': '전체', 'value': 'all'}] + [{'label': m, 'value': m} for m in malls]
    return opts
...
def update_data_table(data, selected_week, cw1, cw2, cw3, cw4, mall_filter, product_filter, selected_columns, page_size):
    ...
    display_cols = [c for c in column_order if c in df_filtered.columns]
...
def update_integrated_viz(data, selected_week, cw1, cw2, cw3, cw4,
                          mall_filter, product_filter, selected_columns, label_mode):
    if not data or not selected_week:
        return go.Figure()
    ...
    if df_filtered.empty:
        fig = go.Figure()
        fig.update_layout(
            title="필터 결과 데이터가 없습니다. (주차/쇼핑몰/상품 필터 조합을 확인해주세요)",
            height=450
        )
        return fig

    # ✅ column-selector 미선택 대비 기본값
    ...
    if not agg_dict:
        fig = go.Figure()
        fig.update_layout(title="표시할 수 있는 컬럼이 없습니다(컬럼 선택/데이터 확인 필요)", height=450)
        return fig
    ...
    if df_agg.empty:
        fig = go.Figure()
        fig.update_layout(title="집계 결과가 없습니다. (필터/컬럼 선택을 변경해보세요)", height=450)
        return fig
    ...
    for idx, col in enumerate(selected_columns):
        ...
        texts, textpos = get_label_text_and_position(df_agg[col].tolist(), col, label_mode, top_n=5)
        ...
        if style['type'] == 'bar':
            fig.add_trace(go.Bar(
                ...
                text=texts,
                textposition=pos if label_mode != 'none' else 'none',
                cliponaxis=False,
                yaxis=style['yaxis'],
            ...
            fig.add_trace(go.Scatter(
                ...
                y=df_agg[col],
                name=col,
                mode='lines+markers+text' if label_mode != 'none' else 'lines+markers',
    ...
    fig.update_layout(
        title="주차별 통합 시각화",
        xaxis=dict(title="주차"),
        yaxis=dict(title="금액 지표", autorange=True),
        yaxis2=dict(title="기타 지표", overlaying='y', side='right', autorange=True),
    )
    fig = apply_common_figure_style(fig, height=450, label_mode=label_mode)
    return fig
        ...
        if compare_weeks:
            ...
            for week in all_weeks:
                ...
                for mall in df_week['쇼핑몰'].unique():
                    ...
                    texts, textpos = get_label_text_and_position(yvals, col, label_mode, top_n=5)
                    ...
                    fig.add_trace(go.Bar(
                        ...
                        text=texts,
                        textposition=textpos if label_mode != 'none' else 'none',
                        cliponaxis=False,
            ...
            fig = apply_common_figure_style(fig, height=520, label_mode=label_mode)
